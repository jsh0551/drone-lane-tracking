...
class PositionPublisher(Node):
    ...
    def timer_callback(self):
        ...
        if self.flag:
            ...
            elif self.step < 2*count:
                self.pose.twist.linear.x = -min(float(2*count-self.step)/4, vel) 
                self.pose.twist.linear.y = 0.0
            elif self.step < 3*count:
                self.pose.twist.linear.x = min(float(self.step-count*2)/4, vel) 
                self.pose.twist.linear.y = 0.0
            else:
                self.pose.twist.linear.x = min(float(count*4-self.step)/4, vel)  
                self.pose.twist.linear.y = 0.0
            self.step += 1
