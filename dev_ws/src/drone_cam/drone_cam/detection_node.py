...
import cv2
from cv_bridge import CvBridge, CvBridgeError         # ROS与OpenCV图像转换类                          # Opencv图像处理库
...
BASE = os.getcwd()[:os.getcwd().find('dev_ws')]
sys.path.append(os.path.join(BASE,"customModule"))
sys.path.append(os.path.join(BASE,"customModule","yolox"))
import DQLL
...
class ImageSubscriber(Node):
    def __init__(self, name):
        ...
        self.sub     # 创建订阅者对象（消息类型、话题名、订阅者回调函数、队列长度）
