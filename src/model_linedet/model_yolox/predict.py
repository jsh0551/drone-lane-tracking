...
def detect():
    ...
    img_dir = opt.dataset_path
    output = "output"
    mkdir(output, rm=True)

    # img_list = get_img_path(img_dir, extend=".jpg")
    img_list = [os.path.join(img_dir,f) for f in os.listdir(img_dir) if ('track' in f) and ('box' not in f)]
    print(img_list,img_dir)
